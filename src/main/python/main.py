...
class ThesaurusPlusUi(QMainWindow):
    def __init__(self, appctxt):
        ...
        self.setWindowIcon(QtGui.QIcon(appctxt.get_resource("Phrog.ico")))
        self.setMinimumSize(700, 400)

        # Initialize re-running dialog box popup
        self.dialog_box = PopUpBoxReRun(appctxt, self)
        self.dialog_config = PopUpBoxConfig(appctxt, self)
...
def main():
    ...
    stylesheet = appctxt.get_resource("customLooks.qss")
    appctxt.app.setStyleSheet(open(stylesheet).read())

    # Show the GUI
    view = ThesaurusPlusUi(appctxt)
    view.show()
